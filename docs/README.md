...
| GUI driven by the matrix | Proposed; nothing built | [A language picker that reads the data](2026-09-10-04-gui-driven-by-the-matrix.md). The demo's language list is twelve locales hardcoded in HTML while the database holds 664 — the exact thing this service exists not to have. Needs `/api/locales`, a picker that takes its list as a prop, endonyms, and a coverage heatmap as a second registry item. |
